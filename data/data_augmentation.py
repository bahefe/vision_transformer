...
# Batchwise augmentation: randomly choose either CutMix or MixUp for each batch.
cutmix_or_mixup = v2.RandomChoice([v2.CutMix(num_classes=10), v2.MixUp(num_classes=10)])

def collate_fn_augment(
    batch: List[Tuple[torch.Tensor, torch.Tensor]]
) -> Tuple[torch.Tensor, torch.Tensor]:
    """Apply either CutMix or MixUp (randomly chosen) to a batch."""
    return cutmix_or_mixup(*default_collate(batch))

def collate_fn_no_augment(
    batch: List[Tuple[torch.Tensor, torch.Tensor]]
) -> Tuple[torch.Tensor, torch.Tensor]:
    """Standard collate without batchwise augmentation."""
